...
class MessageController extends AbstractActionController
{
     ...
     * Constructor.
     */
    public function __construct($entityManager, $sessionManager, MessageManager $messageManager, AuthenticationService $authService)
    {
    ...
    public function getMessagesAction()
    {
        ...
        $chatroom = $this->entityManager->getRepository(Chatroom::class)->findOneBy(['user' => $user->getId()]);
    ...
    public function getChatroomsAction()
    {
        ...
        {
            $msg = $this->entityManager->getRepository(Message::class)->findBy(['chatroom_id' => $chatroom->getId()], ['id'=>'ASC']);
            $messages = [];
            foreach ($msg as $item)
            {
                $messages[] = $item->getData();
            }
            $usr = $this->entityManager->getRepository(User::class)->find($chatroom->getUser());
            $user = $usr->getData();
            $cr = [
                'id' => $chatroom->getId(),
                'messages' => $messages,
